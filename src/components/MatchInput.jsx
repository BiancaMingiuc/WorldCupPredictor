...
export default function MatchInput({ match, score, onChange }) {
  const { team1, team2, date, time } = match;

  const val = (field) => (score?.[field] !== undefined && score?.[field] !== null ? score[field] : "");

  const handle = (field) => (e) => {
    const raw = e.target.value;
    const num = raw === "" ? "" : Math.max(0, parseInt(raw, 10) || 0);
    onChange({ ...(score || {}), [field]: num === "" ? "" : num });
  };
  ...
  return (
    <div className={`rounded-2xl border transition-all duration-300 ${
    ...
    } p-3 sm:p-4 hover:border-white/10`}>

      {/* Date/Time */}
      <div className="text-white/30 text-xs font-medium mb-2.5 flex items-center gap-1.5">
        <span>{date}</span>
      ...
      <div className="flex items-center gap-1.5 sm:gap-3">
        ...
        <div className="flex-1 flex items-center gap-1 sm:gap-2 min-w-0">
          ...
          <span className="text-white text-xs sm:text-sm font-semibold truncate leading-tight">{team1.name}</span>
        </div>
        ...
        <div className="flex-1 flex items-center gap-1 sm:gap-2 justify-end min-w-0">
          <span className="text-white text-xs sm:text-sm font-semibold truncate text-right leading-tight">{team2.name}</span>
          <span className="text-base sm:text-xl shrink-0">{team2.flag}</span>
        </div>
      </div>

      {/* Disciplinary row — stacks vertically on very small screens */}
      <div className="mt-2.5 pt-2.5 border-t border-white/5">
        <div className="flex flex-col xs:grid xs:grid-cols-2 gap-2 sm:gap-3">
          {/* Team 1 disciplinary */}
          <div className="space-y-1">
            <div className="text-white/25 text-[10px] sm:text-xs font-medium">{team1.name}</div>
          ...
          {/* Team 2 disciplinary */}
          <div className="space-y-1">
            <div className="text-white/25 text-[10px] sm:text-xs font-medium xs:text-right">{team2.name}</div>
            <div className="flex items-center gap-1 sm:gap-1.5 xs:justify-end">
